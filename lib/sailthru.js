(function() {
  ...
  SailthruRequest = (function() {
    ...
    SailthruRequest.prototype._http_request = function(uri, data, method, binary_data_params, callback, agent) {
    ...
    SailthruRequest.prototype._api_request = function(uri, data, request_method, binary_data_params, callback, agent) {
      if (typeof binary_data_params === 'function') {
        callback = binary_data_params;
        binary_data_params = undefined;
      }
      ...
      return this._http_request(uri, data, request_method, callback, binary_data_params, agent);
    };
  ...
  SailthruClient = (function() {
    ...
    function SailthruClient(api_key, api_secret, options) {
      this.api_key = api_key;
      this.api_secret = api_secret;
      this.options = typeof options === 'object' && options !== null ? options : {};
      this.api_url = this.options.apiUrl != null ? this.options.apiUrl : false;
      this.agent = this.options.agent != null ? this.options.agent : false;
      ...
      if (this.agent && !(this.agent instanceof http.Agent)) {
        throw Error('If you provide an agent, it must implement http.Agent. ' +
    ...
    SailthruClient.prototype.apiPostMultiPart = function(action, data, binary_data_params, callback) {
      ...
      for (i = 0, len = binary_data_params.length; i < len; i++) {
        ...
        binary_data[param] = multipart.file(data[param], null, stats.size);
        delete data[param];
      }
      _url = url.parse(this.api_url);
      ...
      // The caller should create the agent if they use a corporate proxy.
      // They'll do that by providing the constructed ProxyAgent with a URL.
      if (this.agent) {
        options.agent = agent
      }
      ...
      return rest.post(_url.href + action, json_payload, options).on('done', function(result, response) {
