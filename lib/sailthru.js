(function() {
  var SailthruClient, SailthruRequest, SailthruUtil, USER_AGENT, ProxyAgent, fs, http, https, log, querystring, ref, rest, url,
    child_process, slice = [].slice;
  ...
  SailthruRequest = (function() {
    ...
    // Called by _api_request
    SailthruRequest.prototype._http_request = function(uri, data, method, binary_data_params, callback, proxy_url) {
    ...
    // Called by SailthruClient._apiRequest
    SailthruRequest.prototype._api_request = function(uri, data, request_method, binary_data_params, callback, proxy_url) {
      if (typeof binary_data_params === 'function') {
        callback = binary_data_params;
        binary_data_params = undefined;
      }
      ...
      return this._http_request(uri, data, request_method, callback, binary_data_params, proxy_url);
    };
  ...
  SailthruClient = (function() {
    ...
    function SailthruClient(api_key, api_secret, proxy_url, api_url) {
      this.api_key = api_key;
      this.api_secret = api_secret;
      this.api_url = api_url != null ? api_url : false;
      this.proxy_url = proxy_url != null ? proxy_url : false;
      ...
      // Validate proxy
      if (this.proxy_url) {
        var _proxy = url.parse(this.proxy_url);
        var proxy_protocol_supported = ["http:", "https:", "socks:", "socks4:", "socks5:", "pac+http:"].includes(_proxy.protocol);
        var proxy_used_without_protocol = _proxy.protocol == null;
        if (_proxy.protocol && !proxy_protocol_supported || proxy_used_without_protocol) {
            throw Error('Unsupported proxy protocol.');
        }
      }
     ...
     */
     ...
     // Called by low level api Request functions (get, post, delete)
    SailthruClient.prototype._apiRequest = function(action, data, method, callback) {
      ...
      return this.request._api_request(_url.href + action, json_payload, method, null, callback, this.proxy_url);
    };
    ...
    SailthruClient.prototype.apiPostMultiPart = function(action, data, binary_data_params, callback) {
      // ADD options var
      var _url, options, binary_data, i, json_payload, len, param, stats, value;
      if (typeof binary_data_params === 'function') {
        callback = binary_data_params;
        binary_data_params = undefined;
      }
      ...
      if (this.proxy_url) {
        options.agent = new ProxyAgent(this.proxy_url)
      }

      this.log2(_url.href + action);
      this.log2('MultiPart Request');
      this.log2('JSON Payload: ' + JSON.stringify(json_payload));
      return rest.post(_url.href + action, options).on('complete', function(result, response) {
